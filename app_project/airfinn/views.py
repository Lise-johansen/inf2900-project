...
def login(request):
    ...
    if request.method != "POST":
        return JsonResponse(
            {"error": "Only POST requests are allowed for login."}, status=405
        )
    ...
    print(
        f"data.get.password: {data.get('password')}, data.get.username: {data.get('username')}"
    )
    # Authenticate user
    user = authenticate(
    ...
    if user:
        ...
        response.set_cookie(
            "token", token, httponly=False, secure=False, samesite=False
        )
...
def register(request):
    ...
    if data.get("email") == "":
        return JsonResponse(
            {"error_email": "Requires email to register an account"}, status=400
        )
    if data.get("password1") == "":
        return JsonResponse(
            {"error_password": "Requires password to register an account"}, status=400
        )
    ...
    if data.get("username") == "":
        return JsonResponse(
            {"error_username": "Requires username to register an account"}, status=400
        )
    ...
    # Check if the passwords match
        ...
        fernet.decrypt(encrypted_password1).decode()
        != fernet.decrypt(encrypted_password2).decode()
    ):
    ...
    if user is not None:
        ...
        response = JsonResponse(
            {"token": token, "auth_user": True, "verification_sent": True}
        )
        response.set_cookie(
            "token", token, httponly=False, secure=False, samesite=False
        )
        ...
        # Authentication failed
        return JsonResponse(
            {"success": False, "error": "Not able to create user"}, status=401
        )
...
def send_password_reset_email(request):
    ...
    if user:
        ...
        )  # Assuming you set FRONTEND_URL in your settings
        ...
        # Load plain text content from template
        text_content = render_to_string(
            "password_reset_email.txt", {"reset_link": reset_link, "username": username}
        )
    ...
    if request.method == "POST":
        ...
        if user is not None and token_generator.check_token(user, token):
            ...
            if form.is_valid():
                ...
                return JsonResponse(
                    {"message": "Password reset successfully", "token": access_token}
                )
            else:
...
def contact_us_message(request):
    ...
    try:
        ...
        contact_html_content = render_to_string(
            "contact_us_email.html",
            {"subject": subject, "message": message, "sender_email": from_email},
        )

        # Load plain text content from contact us template
        contact_text_content = render_to_string(
        ...
        autoresponse_html_content = render_to_string(
            "contact_us_autoresponse.html", {"first_name": first_name}
        )

        # Load plain text content from autoresponse template
        autoresponse_text_content = render_to_string(
            "contact_us_autoresponse.txt", {"first_name": first_name}
        )
